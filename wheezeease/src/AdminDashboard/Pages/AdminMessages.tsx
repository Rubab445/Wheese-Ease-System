...
const AdminMessages: React.FC = () => {
  ...
  const [chats, setChats] = useState([
    { id: 0, name: "Ali Ahmad", online: true, history: [{ t: "Bhai report ready hai?", s: false, time: "12:00 PM" }] },
    { id: 1, name: "Sara Khan", online: false, history: [{ t: "Okay, thanks!", s: false, time: "11:30 AM" }] }
  ]);
  ...
  return (
    <div className="admin-wa-full-screen">
      <div className="admin-wa-container">
        ...
        <div className="admin-wa-aside">
          <header className="admin-wa-aside-header">
            ...
            <div className="admin-wa-actions">
              <MoreVertical onClick={() => setShowMenu(!showMenu)} className="clickable-icon" />
              {showMenu && (
          ...
          <div className="admin-wa-contacts-list">
            {chats.map((chat) => (
              ...
              >
                <div className={`pfp ${chat.online ? "online" : ""}`}>{chat.name[0]}</div>
                <div className="chat-meta">
                  <div className="row"><span>{chat.name}</span> <small>12:00 PM</small></div>
        ...
        <div className="admin-wa-chat-window">
          ...
          <header className="admin-wa-chat-header">
            <div className="admin-wa-user-meta">
              <div className="pfp-small">{chats[activeChat].name[0]}</div>
              <div>
          ...
          <div className="admin-wa-chat-body">
            {chats[activeChat].history.map((m, i) => (
              <div key={i} className={`msg-wrapper ${m.s ? "sent" : "received"}`}>
                <div className="msg-bubble">
                  {m.t}
                  <span className="msg-time">{m.time} {m.s && <CheckCheck size={14} />}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <footer className="admin-wa-chat-footer">
            <div className="control-group">
              <Smile onClick={() => setShowEmojis(!showEmojis)} className={showEmojis ? "active-icon" : ""} />
              {showEmojis && (
              ...
              <div className="attach-wrapper">
                <Paperclip onClick={() => setShowAttach(!showAttach)} className={showAttach ? "active-icon" : ""} />
                {showAttach && (
                  <div className="admin-wa-attach-menu">
                    <div className="attach-item"><Image size={20} /> Photos</div>
                    <div className="attach-item"><Camera size={20} /> Camera</div>
                    <div className="attach-item"><FileText size={20} /> Document</div>
                    <div className="attach-item"><User size={20} /> Contact</div>
                  </div>
                )}
              </div>
            </div>
